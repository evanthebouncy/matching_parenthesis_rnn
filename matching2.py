...
cell = rnn_cell.LSTMCell(num_units=num_units, 
                         ...
                         num_proj=num_proj)
cell2 = rnn_cell.LSTMCell(num_units=num_units, 
                         input_size=num_proj, 
                         num_proj=num_proj)
...
state_init2 = tf.Variable(tf.zeros([1, state_size]))
state2 = tf.tile(state_init2, [batch_size, 1])
...
for i in range(0, example_length):
  with tf.variable_scope("cell1") as scope:
    if i > 0:
...
relu1 = tf.nn.relu(tf.matmul(state2, w1) + b1)
